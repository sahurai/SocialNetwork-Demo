    ...
    namespace SocialNetwork.ApplicationLogic.Services
    {
        public class GroupBlockService : IGroupBlockService
        {
            ...
            public async Task<(List<GroupBlock> Blocks, string Error)> GetGroupBlocksAsync(Guid? groupBlockId = null, Guid? groupId = null, Guid? blockerId = null, Guid? blockedId = null)
            {
                try
                {
                    var blocks = await _blockRepository.GetAsync(groupBlockId, groupId, blockerId, blockedId);
            ...
            public async Task<(GroupBlock? Block, string Error)> CreateGroupBlockAsync(Guid groupId, Guid requestingUserId, Guid blockedId)
            {
                try
                {
                    ...
                    {
                        return (null, "Only managers or admins can block users in the group.");
                    }
            ...
            public async Task<(Guid Id, string Error)> DeleteGroupBlockAsync(Guid id, Guid requestingUserId)
            {
                try
                {
                    ...
                    {
                        return (Guid.Empty, "Only managers or admins can delete group blocks.");
                    }
